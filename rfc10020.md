...
In {{fig-exchange-example-blockwise-1}}, the client sends a Non-confirmable GET request to the CoAP group, targeting the resource "log" in the application group "gp1" and requesting a block-wise transfer. All servers reply with a Non-confirmable 2.05 (Content) response including the first block. As the source port number of its response, each server uses its own port number.
...
~~~~~~~~~~~
{: #fig-exchange-example-blockwise-1 title="Example of Non-Confirmable Group Request Starting a Block-Wise Transfer, Followed by Non-Confirmable Responses with the First Block"}
