...
# IESG State: I-D Exists as of 07/08/26
  I-D.tiloca-core-oscore-discovery:
    display: GROUP-OSCORE-DISCOVERY
...
Unsecured group communication relies on the No Security (NoSec) mode, whose use is strongly discouraged and is limited to specific cases (see {{chap-unsecured-groupcomm}}).

Secured group communication is achieved by using Group Object Security for Constrained RESTful Environments (Group OSCORE) {{PRE-RFC10021}}, which in turn builds on OSCORE {{RFC8613}}. This method provides end-to-end application-layer security protection of CoAP messages, by using CBOR Object Signing and Encryption (COSE) {{RFC9052}}{{RFC9053}}.
...
* With reference to {{Section 8.2 of RFC7252}}, it updates the request/response layer for group communication in terms of response suppression (see {{sec-request-response-suppress}} of the present document).

* With reference to {{Section 5.3.1 of RFC7252}}, it updates the token requirements on request/response matching in terms of Token reuse time (see {{sec-token-reuse}} of the present document).
...
The following paragraphs describe the possible relationships between security groups and application groups.
...
The host subcomponent directly defines the IP multicast address of the CoAP group, in the case that the host consists of an IP literal.
The host subcomponent indirectly defines the IP multicast address of the CoAP group, in the case that the host consists of a hostname. Resolving the hostname to an IP address in this case produces the IP multicast address.
...
To create a CoAP group, a configuring entity defines an IP multicast address (or hostname) for the group and optionally a UDP port number in the case that it differs from the default CoAP port number 5683. Then, the configuring entity configures one or more devices as listeners to that IP multicast address, with a CoAP endpoint listening on the CoAP group's associated UDP port. These endpoints/devices are the group members.
...
The following describes how a CoAP endpoint can discover groups by different means, i.e., by using an RD or directly from the CoAP servers that are members of such groups.

#### Discovery Through an RD ### {#sssec-discovery-from-rd}

It is possible for CoAP endpoints to discover application groups as well as CoAP groups by using the RD-Groups usage pattern of the CoRE RD, as defined in {{Section A of RFC9176}}.
...
The following discusses a number of methods to discover application groups and CoAP groups. When discussing the different methods, the two assumptions below are made as an example:

* Application group names are encoded in the path component of group URIs (see {{sec-groupnaming-app}}) and the path segment "gp" is used as the designated delimiter. The same holds for the full examples presented in {{sec-examples-group-discovery}}.

* The type of an application group is encoded in the value of the CoRE Link Format attribute "rt" of a group resource. Also, such values for the attribute "rt" have the semantics "g.\<GROUPTYPE\>", where GROUPTYPE denotes the type of the application group in question. The same holds for the full examples presented in {{sec-examples-group-discovery}}.
...
* A CoAP client can discover all the application groups associated with a specific CoAP group.
   ...
   Through the corresponding responses, the query result is a list of resources at CoAP servers that are members of the specified CoAP group and members of at least one application group associated with the CoAP group. That is, the client gains knowledge of the set of servers that are members of the specified CoAP group and members of any of the associated application groups and, for each of those servers, the name of the application groups where the server is a member and that are associated with the CoAP group.
...
* A CoAP client can discover the CoAP servers that are members of a specific application group, the CoAP group(s) associated with the application group, and optionally the resources that those servers host for each application group.
   ...
   Through the corresponding responses, the query result is a list of resources at CoAP servers that are members of the specified application group and members of at least one CoAP group associated with the application group. That is, the client gains knowledge of the set of servers that are members of the specified application group and, for each of those servers, the CoAP groups where the server is a member and that are associated with the application group. Optionally, for each of those servers, the client also gains knowledge of the resources that the server hosts within the application group.
...
* A CoAP client can discover the CoAP servers that are members of any application group of a specific type, the CoAP group(s) associated with those application groups, and optionally the resources that those servers host as members of those application groups.
   ...
   Through the corresponding responses, the query result is a list of resources at CoAP servers that are members of any application group of the specified type and members of at least one associated CoAP group for each of those application groups. That is, the client gains knowledge of the set of servers that are members of application groups of the specified type and, for each of those servers, the CoAP groups where the server is a member and that are associated with those application groups. Optionally, for each of those servers, the client also gains knowledge of the resources that the server hosts within each of those application groups.
...
A server MAY suppress its response for various reasons given in {{Section 8.2 of RFC7252}}. This document adds the requirement that a server SHOULD suppress the response in the case of error or in the case that there is nothing useful to respond, unless the application related to a particular resource requires such a response to be made for that resource. Such exceptions are application specific and defined by corresponding application policies.
...
As part of a message exchange between the client and any of the servers in the CoAP group, the multiple responses considered above are examples of the more general concept of non-traditional responses elaborated in {{I-D.ietf-core-responses}}.
...
* A client sending a GET or FETCH group request MAY use a response received from a server to satisfy a subsequent sent request intended to that server on the related unicast request URI. In particular, the unicast request URI is obtained by replacing the host subcomponent in the authority component of the group request URI with the source address of the cached response message.
...
In the case that two or more servers in the CoAP group have responded to a past request to the same resource with an identical ETag value, it is the responsibility of the client to handle this case. In particular, if the client
wishes to use a group request for validating a response from server 1 (with ETag value N) and for receiving a fresh response from server 2, there is no way to achieve this using a single group request. This wish could occur if the client has a cached representation for server 1 but has no cached representation for server 2, for example, because the client needed to remove older items from its cache to make space for newer resource representations.
...
* If the NoSec mode is used (see {{chap-unsecured-groupcomm}}), each server treats each received copy of the group request as a different request from a different client. As a result:
   ...
   - From each proxy, the client receives all the responses to the group request that each server has sent to that proxy. Even if the client is able to distinguish the different servers originating the responses (e.g., leveraging the approach used by the realization of proxy specified in {{I-D.ietf-core-groupcomm-proxy}}), the client would receive the same response content originated by each server N times, as relayed by the N proxies.
...
* A server in a constrained network SHOULD only support group requests for resources that have a small representation (where the representation may be retrieved via a GET, FETCH, or POST method in the request). For example, "small" can be defined as a response payload limited to approximately 5% of the IP Maximum Transmit Unit (MTU) size so that it fits into a single link-layer frame in the case that IPv6 over Low-Power Wireless Personal Area Networks (6LoWPAN) (see {{sec-6lowpan}}) is used on the constrained network.
...
for a group size estimate G (which should be chosen conservatively), a target data transfer rate R (which should also be chosen conservatively), and an estimated response size S. Note that S is the estimated average response size for all responding servers for the given group request, not necessarily the known response size of the server's own response to the request. If the Leisure is not computed, then a configured/predefined value is used, which MAY be the default value DEFAULT_LEISURE in case no better or more suitable value is known. In {{RFC7252}}, the default is calculated based on a baseline IEEE 802.15.4 {{IEEE802.15.4}} 6LoWPAN network situation with G=50, S=100, and R=1000; however, this is not explicitly written down.
...
When the group mode is used to protect a response, it is largely cautious to account for 100 additional bytes of security overhead, so that S becomes 200. When the pairwise mode is used to protect a response, it is largely cautious to account for 30 additional bytes of security overhead, so that S becomes 130. Using these new values for S in the calculation yields the following new default parameter values:

* DEFAULT_LEISURE = 10 seconds if the OSCORE group is set to use (also) the group mode.

* DEFAULT_LEISURE = 6.5 seconds if the OSCORE group is set to use only the pairwise mode.

Obviously, the requirement to insert a new leisure period as described above does not apply to retransmissions of a Confirmable separate response (see {{Section 5.2.2 of RFC7252}}) but only to the initial CoAP message transmission when the CoAP retransmission counter is 0 (see {{Section 4.2 of RFC7252}}).

## Observing Resources ## {#sec-observe}
The CoAP Observe Option {{RFC7641}} is a protocol extension of CoAP, which allows a CoAP client to retrieve a representation of a resource and automatically keep its knowledge of this representation up-to-date over a longer period of time. The client gets notified when the representation has changed. {{RFC7641}} does not mention whether the Observe Option can be combined with CoAP (multicast) group communication.
...
A client that sends a group GET or FETCH request with the Observe Option MAY repeat this request using the same Token value and the same Observe Option value in order to ensure that enough (or all) members of the CoAP group have been reached with the request. Repeating a request is discussed in more detail in {{sec-repeating-request}}. This is useful in the case that a number of members of the CoAP group did not respond to the initial request. The client MAY additionally use the same Message ID in the repeated request to avoid members of the CoAP group that had already received the initial request from responding again. Note that using the same Message ID in a repeated request will not be helpful in the case of loss of a response message, since the server that responded already will consider the repeated request as a duplicate message. On the other hand, if the client uses a different, fresh Message ID in the repeated request, then all the members of the CoAP group that receive this new message will typically respond again, which increases the network load.

A client that has sent a group GET or FETCH request with the Observe Option MAY follow up by sending a new unicast CON request with the same Token value and same Observe Option value to a particular server in order to ensure that the particular server receives the request. This is useful if a specific member of the CoAP group did not respond to the initial group request, although it was expected to. In this case, the client MUST use a Message ID that differs from that of the initial group request message.
...
* {{RFC8323}} can be employed to complete block-wise transfers for CoAP group communication, with the limitations discussed in {{sec-block-wise}}.
  ...
  That is, after the first group request containing the Block2 Option is sent over UDP, the following unicast CoAP requests targeting individual servers to retrieve further blocks may be sent over TCP or WebSockets, possibly protected by TLS.
...
* The MLDv2 protocol {{RFC9777}}, which works only if all CoAP servers joining a CoAP group are in link-local communication range of an ingress MPL Forwarder.

* Use of 6LoWPAN ND and EARO as described in {{-multicast-registration}} in a network that supports 6LoWPAN-ND, RPL, and MPL.
...
CoAP group communication can operate in CoAP NoSec mode, i.e., without using application-layer and transport-layer security mechanisms.
...
As mentioned in {{PRE-RFC10021}}, a CoAP endpoint can join an OSCORE group through the realization of a Group Manager that is specified in {{I-D.ietf-ace-key-groupcomm-oscore}} and is based on the Authentication and Authorization for Constrained Environments (ACE) framework {{RFC9200}}.

A CoAP endpoint can discover OSCORE groups and retrieve information to join them through their respective Group Managers by using the method described in {{I-D.tiloca-core-oscore-discovery}} that employs the CoRE RD {{RFC9176}}.
...
* All members of the CoAP group, i.e., the CoAP endpoints that are configured to receive CoAP group messages sent to the particular CoAP group and -- in the case of IP multicast transport -- that are listening to the group's multicast IP address on the group's UDP port.
...
* Group OSCORE prevents any single member of an OSCORE group from being a target for subverting security in the whole group (see {{Section 11.6 of RFC7252}}), even though all group members share (and can derive) the same symmetric-key security material used in the group. In fact, source authentication is always ensured for exchanged CoAP messages, as they can be verified as originating from a specific, identifiable sender within the OSCORE group. This relies on including a signature computed with a node's individual private key (in the group mode) or on protecting messages with a pairwise symmetric key, which is in turn derived from the asymmetric keys of the sender and recipient CoAP endpoints (in the pairwise mode).
...
It follows that, in either case, this replay attack would not accomplish anything on the client; however, it does target the servers in the group that do not implement the optional deduplication based on Message ID.

If Group OSCORE is used, such a replay attack is prevented on all servers, since a client protects each different request with a different Sender Sequence Number value, which is in turn included as Partial IV in the protected message and takes part in the construction of the AEAD cipher nonce. Thus, a server would be able to detect the replayed request by checking the conveyed Partial IV against its own Replay Window in the Recipient Context associated with the client.
...
When CoAP group communication is used in CoAP NoSec
...
The two registrations mentioned above remain in place; however, the Resource Type Link Target Attribute Value "core.gp" and the media type "application/coap-group+json" are not expected to be used by applications and are not used by the present document.
...
In CoAP, device discovery can be implemented by CoAP resource discovery ({{Section 7 of RFC7252}}), sending a request (e.g., a GET request) targeting a particular resource that the sought device class, type, model, or brand is known to respond to. It can also be implemented using CoAP resource discovery and the CoAP query interface defined in {{Section 4 of RFC6690}} to find these particular resources.
...
As defined in {{RFC9176}}, a CoRE RD is a web entity that stores information about web resources and implements REST interfaces for registration and lookup of those resources. For example, a device can register itself to an RD to let it be found by other devices and/or applications.
...
1. Subnets are 6LoWPAN networks; the routers Rtr-1 and Rtr-2 are 6LBRs {{RFC6775}}.
...
* The routers are connected to an IPv6 network backbone (Network Backbone) that is also multicast enabled.  In the general case, this means the network backbone and Rtr-1/Rtr-2 support a PIM-based multicast routing protocol and, e.g., MLDv2 for forming groups.

* A CoRE RD using CoAP (CoAP RD) is connected to the network backbone.
...
Light-1   Light-2    Light-3     Switch     Rtr-1     Rtr-2       RD
 ...
 |          |          |          |          |          |          |
~~~~~~~~~~~
{: #fig-rd-discovery title="RD Discovery via Multicast Request" artwork-align="center"}
...
Note that a group hostname is used in most examples to improve readability. In practice, as discussed in {{sec-groupnaming-app}}, using an IP address literal as the host subcomponent of the group URI can reduce the size of the CoAP request message, if the Uri-Host Option can be elided.
...
{{fig-gname-path-example-2}} provides a different example, where an IPv6 address literal and the default CoAP port number 5683 are used in the authority component to identify a different CoAP group, which yields a compact CoAP request. Also, the resource structure is different in this example.
...
{{fig-gname-query2-example}} provides another example, which considers the second alternative discussed in {{sec-groupnaming-app}}. In particular, the URI query component includes a query parameter "gp" as the designated indicator, whose value is the name of the application group.
~~~~~~~~~~~
...
As a result, the client gains knowledge of the set of servers that are members of the specified CoAP group and member of any of the associated application groups and, for each of those servers, the name of the application groups where the server is a member and that are associated with the CoAP group.
...
Also note that a server could equally well respond with the IPv6 multicast address literal within square brackets instead of the CoAP group name "grp.example". In that case, the client would still gain knowledge of the CoAP group, albeit in a different representation.
...
The next four figures incrementally show an example where the client requests the servers to perform a block-wise transfer.

In {{fig-exchange-example-blockwise-1}}, the client sends a Non-confirmable GET request to the CoAP group, targeting the resource "log" in the application group "gp1" and requesting a block-wise transfer. All servers reply with a Non-confirmable 2.05 (Content) response including the first block. As the source port number of its response, each server uses its own port number. The transfer continues over confirmable unicast exchanges.
...
~~~~~~~~~~~
{: #fig-exchange-example-blockwise-1 title="Example of Non-Confirmable Group Request Starting a Block-Wise Transfer, Followed by Non-Confirmable Responses with the First Block and by Subsequent Confirmable Exchanges over Unicast for the Remaining Blocks"}

After obtaining the first block, the client requests the following blocks separately from each server, by means of unicast exchanges with Confirmable GET requests.

In {{fig-exchange-example-blockwise-2}}, the client requests and receives the following blocks from server A.
...
~~~~~~~~~~~
{: #fig-exchange-example-blockwise-2 title="Example of Unicast Exchanges with Server A to Retrieve the Following Blocks"}

In {{fig-exchange-example-blockwise-3}}, the client requests and receives the following blocks from server B.
...
~~~~~~~~~~~
{: #fig-exchange-example-blockwise-3 title="Example of Unicast Exchanges with Server B to Retrieve the Following Blocks"

In {{fig-exchange-example-blockwise-4}}, the client requests and receives the following blocks from server C.
...
~~~~~~~~~~~
{: #fig-exchange-example-blockwise-4 title="Example of Unicast Exchanges with Server C to Retrieve the Following Blocks"}
